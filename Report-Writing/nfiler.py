...
# Software version
PROG_VER    =    "1.0"
PROG_NAME   =    "Nessus File Reporter"
...
class nessus_parser:
    ...
    def print_statistics(self):
        ...
        print "Total vulns,%d" % (vuln_high + vuln_med + vuln_low + vuln_info)
        print "High vulns,%d" % (vuln_high)
        print "Medium vulns,%d" % (vuln_med)
        print "Low vulns,%d" % (vuln_low)
        print "Info vulns,%d" % (vuln_info)
        print "Available exploits,%d" % (exploits)
    ...
    def print_targets(self, fullinfo=False, delim='|'):
        ...
        for host in IPSet(self._results.keys()):
            host = str(host) # From IPAddress to string
            if fullinfo:
                print "%s%s%s%s%s%s%s%s%s%s%s%s%s" % (host, delim, self._results[host][0]['hostname'], delim, self._results[host][0]['netbios_name'], delim,self._results[host][0]['os'], delim,self._results[host][0]['scan_start'], delim,self._results[host][0]['scan_stop'], delim, self._results[host][0]['mac_address '])
            else:
                print "%s %s" % (host, self._results[host][0]['hostname'])

    def print_org_format(self, cvss_min='2.9', cvss_max='10.0'):
        """
...
if __name__ == "__main__":
    ...
    # Arguments parser
    cmdline = ArgumentParser(description="%s performs information extraction from .nessus files and creates a customized output. (Compatible with Nessus v5 release)" % PROG_NAME, version=PROG_VER, epilog="by the Hedgehogs...")
