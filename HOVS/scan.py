...
url = 'https://192.168.254.10'
# target = '192.168.254.50'
verify = False
token = ''
username = 'scanController'
password = 'fx9mnTfWoWGMyiOf3OAp'
policy_id = 'FreeVulnerabilityScan'
scan_title = (sys.argv[1])
target = (sys.argv[2])
...
if __name__ == '__main__':
    ...
    policy_id = policies['PCI Quarterly External Scan']
    scan_data = add(scan_title, 'Create a new scan with API', target, policy_id)
    scan_id = scan_data['id']

    print('Updating scan with new targets.')
    update(scan_id, scan_data['name'], scan_data['description'], target)
